  ...
  return (
    <nav className="flex-between w-full mb-16 pt-3">
        ...
        {session?.user ? (
          <div className="flex gap-3 md:gap-5">
            ...
            <Link href="/profile" className="flex gap-2 flex-center">
              <Image src={session?.user.image} width={37} height={37} alt="User Profile Image" className="rounded-full" />
            </Link>
          </div>
        ) : (
          <>
      ...
      <div className="sm:hidden flex relative">
        {session?.user ? (
          <div className="flex">
            <Image src={session?.user.image} width={37} height={37} alt="User Profile Image" className="rounded-full cursor-pointer" onClick={() => setToggleDropdown((prev) => !prev)} />
            {toggleDropdown && (
